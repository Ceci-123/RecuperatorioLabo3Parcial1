...
function CalcularEdadPromedio() {
  let acumulador = 0;
  let arrayFiltrado = arrayPersonas.filter((element) =>
    FiltrarPorComboBox(element)
  );
  acumulador = arrayFiltrado.reduce(
    (sumaParcial, a) => sumaParcial + a.edad,
    0
  );
...
class Heroe extends Persona {
  ...
  toString() {
    return `${super.toString()}alterEgo: ${this.alterEgo} - Ciudad ${
      this.ciudad
...
class Villano extends Persona {
  ...
  toString() {
    return `${super.toString()}Enemigo: ${this.enemigo} - Robos ${
      this.robos
